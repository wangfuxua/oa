...
<?php }?>
	<form id="replypostform_<?=$pid?>" name="replypostform_<?=$pid?>" method="post" action="thread_misc.php?act=reply&pid=<?=$pid?>&fid=<?=$fid?>&tid=<?=$_GET['tid']?>">
		<table>
			<tr>
				...
				<td>
					...
					<textarea id="message_<?=$pid?>" name="message"  style="width:98%;" onkeydown="ctrlEnter(event, 'posteditsubmit_btn');" <?php if($post['isthread']){?>rows="18"<?php }else{?>rows="8"<?php }?>>[quote]引用<?=$post['username']?>的回复：<?=$post['message']?>[/quote]</textarea>
				</td>
			</tr>
			<tbody id="editwebimg">
				<tr>
			...
			<tr>
				...
				<?php if($inajax){?>
				<input type="button" name="posteditsubmit_btn" id="posteditsubmit_btn" value="提交" class="submit" onclick="ajaxpost('replypostform_<?=$pid?>', '<?=$pid?>', 'post_reply')" />&nbsp;
				<?php }else{?>
