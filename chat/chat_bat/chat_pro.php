...
$create_room=isset($_REQUEST['create_room'])?$_REQUEST['create_room']:''; 
if($create_room=='1'){
?>
...
<table style="font-size:12px;border:1px solid #2e558a;background:#d2dbeb;margin-top:100px;width:400px;height:160px;" align="center">
<tr><td colspan="2" align="center" height="20"><td><tr>
<tr><td colspan="2" align="center"><a href="<?=$to_dir?>/chat_pro.php?create_room=3&passagain=1&room_name=<?=$room_name?>">创建成功！进入会议室</a><td><tr>
...
<table style="font-size:12px;border:1px solid #2e558a;background:#d2dbeb;margin-top:100px;width:400px;height:160px;" align="center">
	...
	<form action="../chat_pro.php?create_room=4" method="post">
	 ...
	 while($rows=mysql_fetch_array($user_priv)){
		 if($rows['USER_PRIV']=='1'){
			...
			while($rows2 = mysql_fetch_array($chat_list)){
				echo "<tr><td align=center><a href=\"".$rows2['ROOM_DIR']."/chat_pro.php?create_room=3&room_name=".$rows2['SUBJECT']."\"><font color=\"green\">".$rows2['SUBJECT']."</font></a></td><td align=center><a href=\"chat_pro.php?room_id=".$rows2['CHAT_ID']."&create_room=6\" onclick=\"javascipt:return(confirm('您确定要删除这个讨论组？'))\"><font color=\"red\">删除</font></a></td></tr>";
}?>
...
<?php
		...
		/*会议室列表*/
		$chat_list = mysql_query("select * from `chatroom` order by `CHAT_ID`");
		 echo "<table><tr><td>会议室列表</td></tr><tr><td><a href=\"chatroom/chat.php\">进入开放会议室</a></td></tr>";
