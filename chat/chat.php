...
"title"=>"欢迎进入会议室",
//第一个到会议室的欢迎
"firstone"=>"<span style='color:#16a5e9;'>欢迎进入会议室!</span>",
//当信息有禁止内容时显示
"ban"=>"I am a pig!",
//关键字
"keywords"=>"欢迎进入会议室",
...
<div class='contents' style='height:300px;' id='div_contents'>连接中...</div>
		...
		while($rows = mysql_fetch_array($chat_list)){
		 echo "<tr><td><a href=\"".$rows['ROOM_DIR']."/chat_pro.php?create_room=3&room_name=".$rows['SUBJECT']."\">".$rows['SUBJECT']."</a></td></tr>";
		}
		 echo "</table>";
...
名字:<input type=text class="inputtext bg" size=8 id='chat_user' value='<?=$user?>' />&nbsp;
...
&nbsp;<a class='mybut' style='width:75px;font-size:12px;border:1px solid #666;padding:3px;' href="chat_pro.php?create_room=1&username=<?=isset($_GET['username'])?$_GET['username']:''?>">创建会议室</a>&nbsp;<a class='mybut' style='width:75px;font-size:12px;border:1px solid #666;padding:3px;' href="chat_pro.php?create_room=5&username=<?=isset($_GET['username'])?$_GET['username']:''?>">会议室管理</a>
</div>
