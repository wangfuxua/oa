...
var $dp, WdatePicker; (function() {
    ...
    if (!V[N].docMD) {
        ...
        C(T[N], "onmousedown", B);
        T[N].docMD = true
    } C(V, "onunload", function() { $dp.hide() });
    function Q(_) {
        T.$dp = T.$dp || {}; for (var $ in _) T.$dp[$] = _[$];
        return T.$dp
    } function C(A, $, _) {
    ...
    function J() {
        ...
        if (F.substring(0, 1) != "/" && F.indexOf("://") == -1) {
            E = T.location.href; C = location.href;
            if (E.indexOf("?") > -1) E = E.substring(0, E.indexOf("?"));
            if (C.indexOf("?") > -1) C = C.substring(0, C.indexOf("?"));
            var _ = "", D = "", A = "", H, G, B = "";
            for (H = 0; H < Math.max(E.length, C.length);
H++) if (E.charAt(H).toLowerCase() != C.charAt(H).toLowerCase()) {
                G = H; while (E.charAt(G) != "/") {
                    if (G == 0) break;
                    G -= 1
                ...
                break
            } if (_ != "") for (H = 0; H < _.split("/").length; H++) B += "../"; if (D != "") B += D + "/";
            F = E.substring(0, E.lastIndexOf("/") + 1) + B + F
        } $.$dpPath = F
    } function K(C, $, D) {
        var B = V[N], E = B[A]("HEAD").item(0), _ = B.createElement("link");
        _.href = C; _.rel = "stylesheet";
        _.type = "text/css";
        if ($) _.title = $;
        if (D) _.charset = D; E.appendChild(_)
    }
    function X($, _) { C($, "onload", _) }
    function E($) {
        $ = $ || T; var B = 0, _ = 0;
        while ($ != T) {
    ...
    function B(_) {
        ...
        if ($dp && $dp.dd && $dp.dd.style.display == "block" && src != $dp.el) {
            ...
            if ($ != null) {
                $dp.$w.hideSel(); if ($ != "" && !$dp.readOnly) B.date.loadFromDate(B.splitDate($, B.dateFmt)); if ($ == "" || (B.isDate(B.date) && B.isTime(B.date) && B.checkValid(B.date))) { B.mark(true); if ($ != "") B.update(); else B.setRealValue(""); $dp.hide() } else B.mark(false)
            } else $dp.hide()
        }
    } var O = [];
    function W() {
    ...
    true)
            ...
            var B, A = _();
            if (A) { B = A.srcElement || A.target; A.cancelBubble = true }
            C.el = $dp.$(C.el || B);
    ...
    function I(G, A) {
        ...
        else $dp[F] = G[F]; $dp.elProp = $dp.el && $dp.el.nodeName == "INPUT" ? "value" : "innerHTML";
        ...
        if (!$dp.dd || $dp.eCont || ($dp.lang && $dp.realLang && $dp.realLang.name != $dp.lang)) {
            ...
            if ($.$dpPath == "") D(_);
            var B = "<iframe src=\"" + $.$dpPath + "DatePicker.htm\" frameborder=\"0\" border=\"0\" scrolling=\"no\"></iframe>";
            if ($dp.eCont) {
                $dp.eCont.innerHTML = B; X($dp.eCont.childNodes[0], W)
            } else {
        ...
        function C() {
            ...
            if (isNaN(B)) {
                ...
                else B = A.top + $.topM + G.bottom; B += R ? -1 : 1
            }
            else B += A.top + $.topM;
