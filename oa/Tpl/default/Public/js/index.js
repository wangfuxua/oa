...
function showTabs(id) {
    $("#leftTab ul a").removeClass("TabOn");
    if (id != "online") {
        $("#left1").addClass("TabOn");
        $("#left1_con").show();
        //$("#left2_con").find("iframe").contents().empty();
        $("iframe[id='userOnline']").attr("src","");
        $("#left2_con").hide();
    } else {
    $("#left2").addClass("TabOn");
    $("#left1_con").hide();
    $("#left2_con").show().end().find("iframe").attr("src", "__APP__/userOnline/index");
    }
}
//////////////////////////////////////////
...
$(document).ready(function() {
    ...
    //intervalRefresh = setInterval("Refresh()", 5000);
    ...
    checkMenuClick();
        ...
        $("#indexSearch").find("form").submit(function() {
            var _val = $("input[name='indexSearchKey']").val();

            tabCreate("search", "搜索结果", "/index.php/index/indexSearch/key/" + _val + "");
        });
...
function Refresh() {
    ...
    $.post("/index.php/Refresh/index", { test: "" }, function(s) {
        ...
        if (s.msg.ucount != "" && s.msg.ucount != null) {
            setUserCount(s.msg.ucount);
        }
...
function refreshUser() {
    var _box = $("iframe#userOnline");
    var _url = "/index.php/userOnline/index";
}

function checkUserPannel() {
    var _box = "";
    if (_box.css("display") == "block") {
        setCookie("checkState", "start", null, null, null, false);
    }
    setCookie("checkState", "", null, null, null, false);
}
...
// 监测菜单点击事件
    function checkMenuClick() {
        ...
        link.mouseup(function() {
            ...
            tabCreate(id, tab.text(), tab.attr("href"));
            
        });

    }
    function tabCreate(id,text,url) {
        var box = $("#tablist .tabsBox ul");
        var list = box.find("li#tab_" + id);
        if (list.length > 0) {
            list.find("a").attr("href", url);
            var Main = $("iframe[name='icontent']");
            var tab = $("#" + id);
            Main.attr("src", tab.find("a").attr("href"));
        } else {
            box.append("<li class='current' id='tab_" + id + "' ondblclick='removeTabs(\"tab_" + id + "\")'><span onclick='activeTabs(\"tab_" + id + "\")' style='width:'><a href='" + url + "' target='icontent' title='" + text + "'>" + text + "</a></span><em onclick='removeTabs(\"tab_" + id + "\")'>x</em></li>");
        }
        activeTabs("tab_" + id);
