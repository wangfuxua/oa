...
sTree.prototype.node = function(node, nodeId) {
	...
	if (node.url) {
	  ...
	  if(this.config.ajax){
           str += '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node')+'"' + 'onclick=';
           str+='loadContents("'+node.url+'") ';//'") '  有空格
		   str += ' onmouseover="window.status=\'' + node.url + '\';return true;" onmouseout="window.status=\'\';return true;" ';
		 if (node.title) str += ' title="' + node.title + '"';
		 if (node.target) str += ' target="' + node.target + '"';
		 if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc))
			str +=this.obj+' style="cursor:pointer"';
		    str += '>';
    ...
    if (node.oo) {
        str += '<p id="' + node.id + '" title="'+node.name+'" name="' + node._userState + '" class="onlinebar"><a href="javascript:">聊天</a> <a href="javascript:">短信</a> <a href="javascript:">邮件</a></p>';
    }
	str += '</div>';
	// --------- 以上是节点面板 --------
...
function loadContents(urls){
	$('#ger-info').css({textAlign:'center',padding:'50px'})
	 $('#ger-info').html("<img src='/oa/Tpl/default/Public/images/ico/loading3.gif' height='28' width='28' align='absmiddle' style='vertical-align:middle'/>   正在加载……")
		$.ajax({
			   type:'get',
			   url:urls,
			   success:function(requstctn){
				   $('#ger-info').css({textAlign:'left',padding:'0'})
				  $('#ger-info').html(requstctn)
