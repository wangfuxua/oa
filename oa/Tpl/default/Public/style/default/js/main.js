...
Example:
   createHeader({
        ...
        Active:1,
        Toolbar:[
            { Title: "帮助", Url: "http://help.7e73.com", Icon: "", IconCls:"", Cls:"", Target:"_blank",TextHide:true , Action:""}
        ],
        Items: [
...
function createHeader(obj) {
    ...
    if (obj.Title != "" && obj.Title != null) {
        _Html += '<div id="titbar"><h2><b class="' + obj.IconCls + '"></b>' + obj.Title + '</h2>'
         ...
         _Html += addTabs(obj.Items, obj.Active);
...
function addToolbar(items) {
    var _Html = '',_icon='',_Text='',_Act='';
    if (items != "") {
        _Html += '<div class="toolbar"><ul>';
        $(items).each(function(i) {
            if (items[i].Action != '' && items[i].Action != null) {
                _Act = 'onclick=' + items[i].Action;
            } else {
                _Act = '';
            }
            ...
            _Html += '<li class="' + items[i].Cls + '"><a href="' + items[i].Url + '" title="' + items[i].Title + '" target="' + items[i].Target + '" ' + _Act + '>' + _icon + _Text + '</a></li>';
        });
        _Html += '</ul></div>';
