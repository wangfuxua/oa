...
var colValue = {
    id: "",
    style: "",
    desktop: [[["news", "公司新闻"], ["email", "OA内部邮件"]], [["Calendar", "日程安排"], ["notify", "公告通知"]], [["flow", "待办事宜"]]]
};
var sv = [];
$.post("/index.php/Mytable/getSet", "", function(d) {
	_d1=d.replace("代办","待办");
	_d2=_d1.replace(",内部邮件",",OA内部邮件");
    setCookie("oaDesk", _d2, null, null, null, false);
}, "string");
//setCookie("oaDesk", "", null, null, null, false);


var aa = getCookie("oaDesk") || "|/news,公司新闻/email,OA内部邮件|/Calendar,日程安排/notify,公告通知|/flow,待办事宜/url,常用网址";
var _aa1=aa.replace("代办","待办");
var _aa2=_aa1.replace(",内部邮件",",OA内部邮件");
var cc = aa.split("|");
...
$(document).ready(function() {
    ...
    // 初始化Drag的布局,读取cookie中存放的设置
    $(".layer-tit").css("cursor", "");
...
function customToggle() {
    ...
    btn.addClass("on");
    box.show(); /*slideToggle("fast")*/
    funbarShow();

    setDD("ok");
    
    initDrag();
    $(".layer-btn").show().css("cursor", "move");
    setDomHeight("main", 60);
}
function customHide() {
    var btn = $("#custombtn a");
    var box = $("#custombox");
...
function funShow(num) {
    ...
    // 展开box
    box.hide();
    box.eq(num).show(); /*slideToggle("normal");*/
}
// 隐藏 模块设置
function funboxHide() {
...
var Drag = {
    ...
    "start": function(e) {
    // 2009-03-20 增加：利用cookie判断，是否允许桌面拖动
    if (isDD()) {
        $(".layer-tit").css("cursor", "move");
...
///// 添加ok到cookie中
function setDD(val) {
    setCookie("oaDeskCustom", val, null, null, null, false);
}
...
function creatItem(id, box, title) {
    var Html = '<div class="item" id="column-' + id + '">'
                        ...
                        + '         <div class="layer-btn">'
                        + '                 <a href="javascript:" title="缩小" onclick="layerboxToggle(\'' + id + '\')"><img src="/oa/Tpl/default/Public/style/default/images/ico/ico_arrow_bottom.gif" alt="缩小" /></a>'
                        + '                 <a href="javascript:" title="关闭" onclick="layerboxClose(\'' + id + '\')"><img src="/oa/Tpl/default/Public/style/default/images/ico/ico_close.gif" alt="关闭" /></a>'
                        + '         </div>'
