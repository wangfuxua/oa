...
class WebimChatAction extends PublicAction {
	...
	public function sendMsg(){
        ...
        if ($_REQUEST[send]) {
			...
			*/
			...
			$data[uid] = $this->_uid;
			$data[user_name] = $this->LOGIN_USER_ID;
			$data[to_uid] = $to_uid;
			$data[to_user_name] = $_POST['to_user_name'];
			$data[msg] = $msg;
			$data[dis_time] = date("H:i:s",$this->CUR_TIME_INT);
			$data[flag_new] = 1;
			$data[cre_time] = $this->CUR_TIME_INT;
			$daoim = new WebimChatModel();
			//$daoim = D("WebimChat");
			$id=$daoim->add($data);
			if ($id) {
			   $re=array("flag"=>1);
	...
	public function refresh(){
		...
		//$map="to_user_name='$this->LOGIN_USER_ID' and user_name='$value' and cre_time>($this->CUR_TIME_INT-5)";
