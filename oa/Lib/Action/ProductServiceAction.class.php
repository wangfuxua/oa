 ...
 * Date: 2009-01-09
 * */
class ProductServiceAction extends PublicAction {
	...
	public function create(){
		...
		$this->assign('productService_res', $user_res);
		$this->display();
	}
	...
	public function view(){
		...
		/**获取指定id的潜在客户信息及地址信息*/
		$productService = D('ProductService');
		$productService_res = $productService->find($id);
		
		/**获取当前记录的相关创建人，最后修改人以及负责人*/
		$user =  D('User');
		$builder_res = $user->where("uid='".$productService_res['builder_id']."'")->find();
		$modify_res = $user->where("uid='".$productService_res['last_modifier_id']."'")->find();
		$manager_res = $user->where("uid='".$productService_res['manager_id']."'")->find();
		
		$this->assign('productService_res',$productService_res);
	...
	public function edit(){
		...
		$user =  D('User');
		$manager_res = $user->where("uid='".$productService_res['manager_id']."'")->find();
		$productService_res['uid'] = $manager_res['uid'];
		$productService_res['USER_NAME'] = $manager_res['USER_NAME'];
	...
	public function update(){
		...
		$productService = D('ProductService');	
		if ($productService->create()){
			$productService->where("id='$id'")->save();
		}
		$this->redirect("view/id/$id");
	}
	...
	public function lists(){
		...
		$count	= $productService->count($condition);
		$rows	=	10;
		$p		=	new Page($count,$rows);
		$list	=	$productService->findAll($condition,'*',$sort,$p->firstRow.','.$p->listRows);
		$page	=	$p->show();
		$xml_grade = new xml("service_grade.xml", 'item');
		
		$this->assign('xml_grade',$xml_grade->array); // 类型记录集
		$this->assign('productService_res',$list);
	...
	public function del(){
		...
		$productService = D('ProductService');
		$productService->delete("id in($ids_string)");
		$this->redirect('lists');
	}
}
?>