...
<?php if(is_array($list)): ?><?php $i = 0;?><?php $__LIST__ = $list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = (($i % 2 )==0)?><tr>
          <td align="center"><input type="checkbox" name="email_select" value="<?php echo (is_array($vo)?$vo["EMAIL_ID"]:$vo->EMAIL_ID); ?>" /><?php echo (is_array($vo)?$vo["EMAIL_ID"]:$vo->EMAIL_ID); ?></td>
