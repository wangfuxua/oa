...
$(function(){
        ...
        Cls: "",
        Active: <?php if($row[DZYH_ID] != ''): ?>3<?php elseif($a == 'form'): ?>2<?php endif; ?>,
        Toolbar: [
        ...
        Items: [
			...
			<?php if($row[DZYH_ID] != ''): ?>,
            { Title: "修改低值易耗品", Url: "/index.php/Dzyh/form", Cls: "", IconCls: "ico ico-view" }<?php endif; ?>
        ]
    });		   
});
...
<table>
<col width="120px" />
  <form action="/index.php/Dzyh/formsubmit" method="post" name="form1" id="form1">
   <tr>
    <th>低值易耗品名称：</th>
    <td>
   ...
   <tr>
    ...
    <td>
        ...
        <img src="/oa/Tpl/default/Public/images/ico/calendar.png" alt="选择时间" style="cursor:hand" onClick="WdatePicker({el:$dp.$('KSSYRQ'),dateFmt:'yyyy-MM-dd'})"  />
    </td>
   </tr>
   <tr>
    <th>预计使用年限：</th>
   ...
   <tr>
    <th  colspan="2" align="center">
        <input type="hidden" name="DZYH_ID" value="<?php echo ($row[DZYH_ID]); ?>"> 
        <?php if($row[DZYH_ID]): ?><button type="submit" value="修改" class="btnFnt" title="修改低值易耗品" name="button">修改</button>
        <?php else: ?>        
        <button type="submit" value="添加" class="btnFnt" title="增加低值易耗品" name="button">添加</button><?php endif; ?>
        <button type="button" value="返回" class="btnFnt" onClick="location='/index.php/Dzyh/manage'">返回</button>
    </th>
   </tr>
   </tfoot>
