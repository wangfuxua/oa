...
function delete_attach(ATTACHMENT_ID,ATTACHMENT_NAME)
{
  ...
  $.ajax({
		type: "POST",
		url: "/index.php/manage/deleteAttach/NOTIFY_ID/<?php echo (is_array($ROW)?$ROW["NOTIFY_ID"]:$ROW->NOTIFY_ID); ?>/ATTACHMENT_ID/"+ATTACHMENT_ID+"/from/edit/ATTACHMENT_NAME/"+ATTACHMENT_NAME,
		success: function(){
		alert("删除成功！");
...
$(function(){
        ...
        Items: [
            ...
            { Title: "修改公告", Url: "#", Cls: "", IconCls: "ico ico-edit" }
        ]
    });		   
});
</script>
<body>
	...
	<div class="KDStyle" id="main">
		<form enctype="multipart/form-data" action="/index.php/manage/update"  method="post" name="form1">
			<table>
				<colgroup>
					<col width="120"></col>
					<col width=""></col>
				...
				<tr>
					...
					<td><textarea cols="20" name=TO_NAME id="TO_NAME" rows="2" class="selectuser" wrap="yes" readonly><?php echo (is_array($ROW)?$ROW["allname"]:$ROW->allname); ?></textarea> 
					<button name="Abutton2"  type="button" onclick="setInput('TO_ID','TO_NAME','right_show1','true')">添加</button>
				...
				<tr>
					...
					<td>
						...
						<input type="hidden" name="oldattname[]" value="<?php echo ($vo[filename]); ?>"></span><br><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?><?php endif; ?>
				...
				<tr>
					...
					<td class="clearTable">
						<!-- 这里插入编辑器 -->	
						<textarea cols="" id="CONTENT" name="CONTENT" style="height:205px"><?php echo (is_array($ROW)?$ROW["CONTENT"]:$ROW->CONTENT); ?></textarea>
					</td>
				</tr>
				<tfoot>
					<tr>
						<th colspan="2">
							...
							<button name="Abutton1" onClick="sendForm()">修改</button>
