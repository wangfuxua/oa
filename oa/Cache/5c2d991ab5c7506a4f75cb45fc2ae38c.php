...
<?php if(is_array($list)): ?><?php $i = 0;?><?php $__LIST__ = $list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = (($i % 2 )==0)?><tr>
          ...
          <td><?php echo (getList_name(is_array($vo)?$vo["TO_ID"]:$vo->TO_ID)); ?></u></td>
          <td><span class="til">
          				<?php if($vo[READ_FLAG] == 1): ?><a href="#" onclick='javascript:$("#mail-act").load("/index.php/Email/add/EMAIL_ID/<?php echo (is_array($vo)?$vo["EMAIL_ID"]:$vo->EMAIL_ID); ?>/BOX_ID/<?php echo (is_array($vo)?$vo["BOX_ID"]:$vo->BOX_ID); ?>")'><?php echo (is_array($vo)?$vo["SUBJECT"]:$vo->SUBJECT); ?></a>
						 <?php else: ?>
						<a href="#" onclick='javascript:$("#mail-act").load("/index.php/Email/add/EMAIL_ID/<?php echo (is_array($vo)?$vo["EMAIL_ID"]:$vo->EMAIL_ID); ?>/BOX_ID/<?php echo (is_array($vo)?$vo["BOX_ID"]:$vo->BOX_ID); ?>")'><strong><?php echo (is_array($vo)?$vo["SUBJECT"]:$vo->SUBJECT); ?></strong></a><?php endif; ?>
          </span></td>
...
$.ajax({
   	type:'post',
  	url:'/index.php/Email/deleteall/from/outbox'
})	
$("#page-Num").html("")
}
else return
