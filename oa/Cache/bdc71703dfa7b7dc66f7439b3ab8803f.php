...
<div id="active" class="active">
  ...
  <div id="mail-centens" style="border:none">
      ...
      <div class="tab-ft">
        ...
        <div class="btm-act">
			<span>转移到：<select name="BOX_IDS" onchange="change_box();" style="border:1px solid #ccc">
...
function view_mail(mailid){
		   $("#mail-act").html('');
		   $("#mail-act").load("/index.php/WebMail/view/?setid=<?php echo ($setid); ?>&from=inbox&idx="+mailid);
}
		...
		for(var j=0; j<gh.length; j++){
			if(gh[j].checked==true){
		     ...
		     if(py.tagName!='tr')
			   ...
			   $.ajax({
			   	type:'post',
			   	url:'/index.php/WebMail/delete/folder/inbox/idxs/'+gh[j].value+','
			   })			   			   			   
			 }
		  }		 
		  return 
